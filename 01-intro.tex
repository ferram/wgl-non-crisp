...
According to the above considerations, in \cite{FerFioRod:2025} we
presented the logic $\GWCL$ characterized by crisp witnessed G\"odel
...
In \cite{FerFioRod:2025} we have presented a refutation calculus based
on constraints for $\GWCL$, inspired to the tableau calculus presented
in~\cite{BilkovaFK:22}. 

In this paper this paper we introduce the fuzzy (non crisp) logic
$\GWL$, which is semantically characterized by witnessed
G\"odel Kripke models
%%, a restriction of $\GM$-models in which the
%%value of a modal formula $\Box \a$ or $\Diamond \a$ at a world $w$
%%coincides with (and is thus witnessed by) the value of $\a$ at some
%%world accessible from $w$.  
We then introduce a refutation calculus $\Calcw$ for $\GWL$ and
establish its soundness and completeness with respect to witnessed
models. On the basis of $\Calcw$, we design a terminating decision
procedure with countermodel generation, based on a standard backward
proof-search strategy. As a direct consequence of this decision
procedure, $\GWL$ enjoys the finite model property, since every
non-provable formula admits a finite countermodel effectively
extracted from a failed derivation.  Finally, we provide an
implementation of the procedure, called \texttt{gwref} (available
at~\cite{gwrefProver}). 
