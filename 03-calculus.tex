...
refutation calculus $\Calcw$ for $\GWL$,
namely, a calculus to certify that a formula is not valid in any
$\GWM$-model.
The constraint language $\LC$ is
...
e.g.,~\cite{TroSch:00}).
We remark that $\Calcw$ has the \emph{subformula
    property}, namely: if $\Tcal$ is a tree of $\Calcw$ having $\G_0$ as
...
\end{proposition}
\begin{proof}
The relation $\precm$ is well-founded and can be used to instantiate  $\prec$
in the definition of strong termination
(see Lemma~\ref{lemma:rulesDec} in the Appendix). 
\end{proof}
...
$\M\models \G$ can be built; we call $\M$ a \emph{countermodel} for $\G$,
...
Let $\s$ be a solution to $\Atmp{\G_n}$. Then, 
$\Mod{\Atmp{\G_n},\s}$ is a countermodel for $\s_0$.
