...
thus,  $w:\a > w:\b$ is replaced by $w:\a > w: q$,
moreover we need the constraint $w:\b\leq w:q$,
...
both rules introduce the new label $w_1$.
...
By the induction hypothesis,
  $\M\models_\Ical \chi_1$, hence $r_1\lt\ \Ical(t)$.
...
Finally, let us assume  $\wg{\chi}>0$ and   $\chi\in \G_n$;
then  either $\chi= w:\Box\a\gt t$ or  $\chi= w:\Diam\a\lt t$.
...
$\M\models_\Ical R(w,w') \to w':\a\gt t$, for every $w'\in W$.
  \end{enumerate}
  Let us assume that $w'$ occurs in $\G_0$. Then, $\G_n$ does not contain
 any atomic term of the form $R(w,w')$ since, in such a case, $w'$ should be a new label.
 As a consequence, the substitution $\s$ is not defined on $R(w,w')$,
 thus the value of $R(w,w')$ is 0.
...
let  $k > 0$  such that $w'$ is introduced in the set $\G_{k}$, namely:
 $w'$ occurs in  $\G_{k}$
...
Since the application of $\rho$ is plain,
no rule applied in the sub-branch $\stru{\G_{k},\dots,\G_n}$
can introduce in the premise a constraint  $u\abstractorder t'$ such that
$u$ has label $w$;
this implies  that   $\chi\in \G_{k-1}$.
Since $\Phibd{\G_{k-1},w,w'}\subseteq \G_{k}$, we get
$R(w,w') \to w':\a\gt t \in\G_{k}$,
