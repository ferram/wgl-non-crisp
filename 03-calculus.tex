...
Let $e'=\reass{e}{p_0}{\a}$ and $w\in W$; one can prove that
 $e'(w,p_0\tto \a)$ and, for every formula $\varphi$ not containing $p_0$, $e(w',\varphi)=e(w,\varphi)$.
...
such that $\M\models\G$, then there are a premise $\G'$ of $\rho$
and a  \GWM-model $\M'$
  such that $\M'\models\G'$.
\end{lemma}

\begin{proof}
  Let $\rho$ be an application of the rule $\ruleAxat$; since the rule has no premises,
...
we show that  then there are a premise $\G'$ of $\rho$
and a  \GWM-model $\M'$ such that $\M'\models\G'$.
