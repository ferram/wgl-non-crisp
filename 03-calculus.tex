...
$\ruleToLTAt$, having main constraint $w:\a\to p < t$. The premise
...
then $e(\Ical(w),\a) > e(\Ical(w),p)$, $e(\Ical(w), \a\to p) =
e(\Ical(w),p)$ and $e(\Ical(w),p) < \Ical(t)$.  This reasoning cannot
be generalized to any $\b$ since the constraint $w:\a > w:\b$ is not
allowed if $\b\not\in\PV\cup\{\bot\}$.  In the latter case, rule
$\ruleToLT$ introduce a new propositional variable $q$, behaving in
...
\begin{proposition}\label{prop:satBranch}
  ...
  \begin{enumerate}[label=(\roman*), ref=(\roman*),leftmargin=*]
  \item\label{prop:satBranch:1} Let $\s$ be a solution to
    $\Atmp{\G_n}$. Then, $\Mod{\Atmp{\G_n},\s}$ is a discrete
