...
The definitions of \emph{tree} and
\emph{derivation} of $\Calcw$ are the usual ones (see, e.g.,~\cite{TroSch:00}).

\inlinetodo{ We remark that $\Calcw$ has the \emph{subformula
    property}, namely: if $\Tcal$ is a $\Calcw$-tree having $\G_0$ as
...
that $\Calcw$ is a sound and complete refutation calculus for $\GWL$,
where soundness and completeness are formalized as follows:
...
\end{array}    
\]
The multisets  $\G[w]$ (a multiset of constraints) and   $\sizem{\G}$ (a multiset of natural numbers) are defined as follows:
\[
  \G[w] \,=\,\{\,
...
\begin{example}\label{ex:satBrancj}
  ...
  q)$.  The $\Calcw$-tree shown below corresponds to a saturated
  branch $\Bcal=\stru{\G_0,\dots,\G_7}$, where $\G_0=\{w_0:\varphi <
  1\}$ is the root and $\G_7$ is the top multiset.  When the applied rule
  has two premises, the annotation $(l)$ (left) or $(r)$ (right)
  specifies the selected one; in every rule application, the main
  constraint is underlined.
\[\small
  \begin{array}{l}
% Gamma_7
