...
\end{enumerate}
By the induction hypothesis,
  $\M\models_\Ical R(w,w_1)\to w_1:\a\lt t$, hence:
 \[
 e(w_1,\, R(w,w_1)\to w_1:\a)\,\lt\, \Ical(t)
\]
Since $e(w,\Box \a) \leq  e(w_1,\, R(w,w_1)\to w_1:\a)$, we get $\M\models_\Ical  w:\Box\a\lt t$.
...
$\M\models_\Ical R(w,w') \to w':\a\gt t$.
  \end{enumerate}
Note that the label $w'$ does not occur in $\G_0$, thus
there exists  $k \geq 0$  such that $w'$ is introduced in the set $\G_{k+1}$, namely:
 $w'$ occurs in  $\G_{k+1}$
and, for every  $0\leq i \leq k$, $w'$ does not occur in $\G_i$.
Accordingly, $\G_{k+1}$ is the  premise of an application of a  modal rule $\rho$
having conclusion $\G_{k}$.
Since the application of $\rho$ is plain,
no rule applied in the sub-branch $\stru{\G_{k+1},\dots,\G_n}$
can introduce in the premise a constraint  $u\abstractorder '$ such that
$u$ has label $w$;
this implies  that   $\chi\in \G_{k}$.
