...
and $w:p < t$.  This reflects the fact that, given a model $\M$, a
world $w$ in $\M$ and an $\M$-interpretation $\Ical$, if $e(\Ical(w),
\a\to p) < \Ical(t)$, then $e(\Ical(w),\a) > e(\Ical(w),p)$,
$e(\Ical(w), \a\to p) = e(\Ical(w),p)$ and $e(\Ical(w),p) < \Ical(t)$.
This reasoning cannot be generalized to any $\b$ since the constraint
$w:\a > w:\b$ is not allowed if $\b\not\in\PV\cup\{\bot\}$.  In the
latter case, we introduce a new propositional variable $q$, behaving
in $w$ as $\b$, and $w:\a > w:\b$ is replaced by $w:\a > w: q$.  The
...
Fig.~\ref{fig:calc3}) are similar to the corresponding propositional
rules.  The definitions of \emph{tree} and \emph{derivation} of
$\Calcw$ are the usual ones (see, e.g.,~\cite{TroSch:00}).
...
The weigth $\wg{u}$ of a constraint $u$, the weight $\wg{\G}$ of a set
...
$\Bcal$ represents a branch of a tree.
...
\end{cases}
  \end{array}
  \]
  We remark that $\M$ is a discrete $\GWM$-model.  An interpretation
  $\Ical$ of $\Mod{\Gat,\sigma}$ is \emph{canonical} iff $\Ical(w)=w$
  for every label $w$ occurring in $\Gat$.
...
\begin{example}\label{ex:satBrancj}
  Let $\varphi$ be the formula $\Box(p\lor q) \to (\Box p\lor \Diam
  q)$.  The tree shown below corresponds to a saturated branch
...
\begin{proof}
  ...
  an open branch $\Bcal$ with $\G_0$ as root.  Since $\Bs$ is plain,
  the branch $\Bcal$ is saturated, hence a discrete
  countermodel for $\G_0$ can be extracted from $\Bcal$ (see
  Prop~\ref{prop:satBranch}\ref{prop:satBranch:1}).
\end{proof}
