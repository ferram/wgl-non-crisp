...
construction.  Nevertheless, using the above bounds, we can define an
...
The algorithm iteratively tries to check if a model such that
...
backtracking is $O(|\varphi|)$.  If some modal formula occurs in
$\Phi(w)$, then for every modal formula $\alpha$ a successor world
$w'$ is generated. First, a value in $\Qcal$ for $R(w,w')$ is chosen
and $\Phi(w')$ is initialized so that it is consistent with the
assignments in $\Phi(w)$ for modal formulas. Then the procedure is
recursively applied to $w'$, taking into account the assignments in
...
refining$|\varphi|$.  If all the required successors of $w$ can be generated,
...
attempts to construct a countermodel fail, we conclude that
$\varphi\in\GWL$.  One can show that the outlined procedure runs in
PSPACE; see App.~\ref{app:pspace} for further details.

We have implemented both the proof-search and the countermodel
extraction procedure. The prover, named
