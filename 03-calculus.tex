...
This corresponds to the fact that, given a model $\M$, a world $w$ in $\M$ and an  $\M$-interpretation
$\Ical$, if $e(\Ical(w), \a\to p) < \Ical(t)$, then $e(\Ical(w),\a) > e(\Ical(w),p)$
and  $e(\Ical(w),p) <  \Ical(t)$ (indeed,  $e(\Ical(w), \a\to p) =  e(\Ical(w),p)$).
This schema cannot be generalized to any $\b$ since
...
\item $\G$ is \emph{plain} iff 
  only a modal rule can  backward applied to $\G$;
  thus,  every non-atomic constraint in $\G$ has the form
...
$\G_k$ and main constraint $\chi$ (note that $\rho$ is determined by $\chi$).
...
in such cases,  either $\chi= w:\Box\a\gt t$ or  $\chi= w:\Diam\a\lt t$.
...
$\M\models_\Ical R(w,w') \to w':\a\gt t$, for every $w'\in W$.
  \end{enumerate}
  Let us assume that $w'$ occurs in $\G_0$. Note that
 $R(w,w')$ does not occur in $\G_n$,
 otherwise  $w'$ should be a new label,
  against the assumption that $w'$ occurs in $\G_0$.
 As a consequence,  the value of $R(w,w')$ is 0.
To prove~\ref{lemma:count:boxgt}, we have to show  that $\M\models_\Ical 1 \gt t$.
...
Let us assume that  $w'$ does not occur in $\G_0$ and
let  $k > 0$  be such that $w'$ is introduced in  $\G_{k}$, namely:
 $w'$ occurs in  $\G_{k}$
and, for every  $0\leq i < k$, $w'$ does not occur in $\G_i$.
Accordingly, $\G_{k}$ is the  premise of an application of a  modal rule $\rho$
having conclusion $\G_{k-1}$.
...
can introduce in the premise a new constraint  $u\abstractorder t'$ such that
$u$ has label $w$;
this implies  that   $\chi\in \G_{k-1}$.
It follows that $\Phibd{\G_{k-1},w,w'}\subseteq \G_{k}$, hence
$R(w,w') \to w':\a\gt t \in\G_{k}$ and~\ref{lemma:count:boxgt} holds by the induction hypothesis.
