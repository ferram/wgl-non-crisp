...
We only detail some  representative cases, by showing the corresponding rule application.
  ...
  < t$. 
  Let $r=e(\Ical(w),\b)$ and let $\M'=\stru{W,R,e'}$ be the \GWM-model where
  $e'=e[(\Ical(w),q):=r]$.
%  We remark that, for every constraint $\chi$ not containing $q$, $\M'\models_\Ical \chi$ iff $\M\models_{\Ical}  \chi$.
  ...
  Let $r=e(\Ical(w),\b)$ and let
  $\M'=\stru{W,R,e'}$ be the $\GWM$-model where $e'=e[(\Ical(w),q):=r]$.  Since
  $\M\models_{\Ical} \Delta$ and $q$ does not occur in $\Delta$, we get
...
\qquad
  ...
  $r=e(\Ical(w),\a)$ and let $\M'=\stru{W,R,e'}$ be the \GWM-model where
  $e'=e[(\Ical(w),q):=r]$; reasoning as in the previous cases,   we can prove that $\M'\models_{\Ical} \G_1$.
...
\item\label{prop:soundRule:box:3}     
  ...
  %For every $\chi=R(w,w_1)\to w_1 : \b \gt t'\in\Phibd{\G,w,w_1}$,  $\M\models_{\Ical'} \chi$.

% \item\label{prop:soundRule:box:4}     
%    For every $\chi=R(w,w_1)\wedge w_1 : \b \lt t'\in\Phibd{\G,w,w_1}$,  $\M\models_{\Ical'} \chi$.
  \end{enumerate}
