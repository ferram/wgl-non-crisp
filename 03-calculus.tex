...
%% BOX LT  1      
     ...
     \AXC{$R(w,w_1) > 0,\,R(w,w_1)\to w_1:\a \lt t,\,\Phibd{\G,w,w_1},\,\G$}
     % -----------------------------------------
     \RightLabel{$\Box\lt_1$}
     \BIC{$w:\Box\a \lt t,\,\G$}    
     \DP
     \\[4ex]
    %% BOX LT  2      
     \AXC{$R(w,w_1) > 0,\,R(w,w_1)\to w_1:\a \lt t,\,\Phibd{\G,w,w_1},\,R(w,w') > 0,\,\G$}
     % -----------------------------------------
     \RightLabel{$\Box\lt_2$}
     \UIC{$w:\Box\a \lt t,\,R(w,w') > 0,\, \G$}    
     \DP
     \\[4ex]
%% DIAM GT L  1    
     \AXC{$0 \gt t,\,\Phizo{\G,w}$}
     \AXC{$R(w,w_1)>0,\,R(w,w_1)\land w_1:\a \gt t,\,\Phibd{\G,w,w_1},\,\G$}
     % ------------------------------------------
     \RightLabel{$\Diam\gt_1$}
     \BIC{$w:\Diam\a \gt t,\,\G$}    
    \DP
 \\[4ex]
     %% DIAM GT L  2   
     \AXC{$R(w,w_1)>0,\,R(w,w_1)\land w_1:\a \gt t,\,\Phibd{\G,w,w_1},\, R(w,w') > 0,\, \G$}
     % ------------------------------------------
     \RightLabel{$\Diam\gt_2$}
     \UIC{$w:\Diam\a \gt t,\,R(w,w') > 0,\,\G$}    
    \DP
     %%% WHERE ....
     \\[3ex]
     \begin{minipage}{1.0\linewidth}
...
we introduce a new propositional variable $q$ behaving as $\b$ in $w$;
...
Both modal rules introduce a new label $w_1$.
...
\item Let $w: \varphi\abstractorder t$ be the main constraint of the application; 
...
The canonical interpretation of $\Mod{\Gat,\sigma}$ 
is the interpretation $\Ical$ such that
$I(w)=w$ for every label $w$.
...
in the next lemma we show that
 $\M\models_\Ical \G_0$, where $\Ical$ is the canonical interpretation.
...
c = w_0:q_0\qquad\D\;=\;\{\,  R(w_0,w_1) > 0,\;\;   w_0:  \Box(p\lor q) > c,\;\;  w_0: \Diam q  \leq  c,\;\;  c   < 1 \,\}    
  ...
  R(w_0,w_1)> w_1:p,\;\; w_1:p\leq c, \;\;  w_1 : q \geq R(w_0,w_1),\; 1> c,\;\;  R(w_0,w_1)\leq c,\;\;  R(w_0,w_1) > 0,\;\; c < 1  
    \]
The set   $\Atmp{\G}$ is consistent; a solution to $\Atmp{\G}$ is any substitution $\s$ on  $\Atmp{\G}$ such that
    \[
...
$\M \models_\Ical w_0 :\varphi < 1$, where $\Ical$ is the canonical interpretation,
hence $\M$ is a countermodel for $\varphi$, witnessing that $\varphi\not\in\GWL$.
A  solution $\s^\star$ is  given by setting $\sigma^\star(w_1:p) = 0.5$ and
...
\begin{lemma}\label{lemma:count}
  ...
  $\Atmp{\G_n}$.
  Let $\M=\Mod{\Atmp{\G_n},\s}$,  $\Ical$  the canonical interpretation
  of $\M$ 
