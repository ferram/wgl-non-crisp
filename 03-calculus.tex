...
\begin{proof}
  ...
  For the other cases see App.~\ref{app:proofs}.
\end{proof}
...
\begin{proof}
  ...
  Lemma~\ref{lemma:rulesDec} in App.~\ref{app:proofs}).
\end{proof}
...
Note  that $\M$ cannot be constructed incrementally, one branch at a time,
since the constraints generated during the expansion of a branch have
global validity and must be preserved throughout the entire construction.
Nevertheless, using the above bounds, we can define an algorithm to
check if such a countermodel exists without effectively constructing
it. The  key observation is that we can bound the number of values $N$ needed
to define the maps $R$ and $e$ of the countermodel.
In particular, if $\varphi\not\in\GWL$,
there is a countermodel where $R$ and $e$
 take values in
...
possible pair of worlds $w$, $w'$ and every propositional variable $p$
occurring in $\varphi$. The algorithm iteratively tries to check if a
model such that $e(w_0,\varphi)=r_0$ exists, where $r_0\in\Qcal\setminus\{1\}$.
%(where $w_0$ is a world of the model). 
The algorithm maintains, for each generated world $w$, a set $\Phi(w)$ of
assignments  of the form  $e(w,\a)=r$, where $\a$ is a subformula of $\varphi$ and
$r\in \Qcal$,
specifying the intended value $r$ of $e(w,\a)$. We remark that, at each step of the execution,
at most $O(\size{\varphi})$ entries of the map $\Phi$ must be recorded,
and each entry $\Phi(w)$ requires $O(\size{\varphi})$ space.
The algorithm proceeds in two phases. First,
the assignments  $e(w,\a)=r$  in $\Phi(w)$, where $\a$ is non-modal,  
are saturated  by assigning values in $\Qcal$  to the subformulas of $\a$
and updating the assignments in $\Phi(w)$ accordingly.
At the end of this saturation phase,  every formula $\a$  occurring in $\Phi(w)$,
is either atomic or modal. If all the $\a$ are atomic, the procedure
terminates successfully if the assignments in $\Phi(w)$ are coherent
(i.e., $\Phi(w)$ does not assign distinct values to the same formula), otherwise the procedure
backtracks and tries different assignments from $\Qcal$. We point out that the memory space required to
manage backtracking is $O(|\varphi|)$. 
If some modal formula occurs in $\Phi(w)$, then for every modal formula
$\alpha$   a successor world $w'$ is generated. First,
a  value in $\Qcal$ for $R(w,w')$ is chosen and  $\Phi(w')$ is initialized
so that it is consistent with the assignments in $\Phi(w)$ for modal formulas. Then
the procedure is recursively applied to $w'$, taking into account the assignments in
...
$|\varphi|$.
If all the required successors of $w$ can be generated,
 the procedure terminates successfully; 
accordingly,   a $\GWM$-model
such that $e(w_0,\varphi)=r_0$ can be built, hence $\varphi\not\in\GWL$.
Otherwise, the procedure backtracks. 
If all attempts to construct a
countermodel fail, we conclude that $\varphi\in\GWL$.
One can show that the outlined procedure runs in PSPACE; see App.~\ref{app:pspace} for further details.
...
depth-first proof search, relying on the JTabWb engine extended with a
Java implementation of the $\Calcw$ rules together with a basic
...
We emphasize that the calculus $\Calcw$ and the associated proof-search
strategy can be smoothly adapted to treat the logic
$\GWCL$~\cite{FerFioRod:2025}, characterized by crisp $\GWM$-models, by
refining the notion of consistency for a set of atomic constraints
$\Gat$: a solution $\s$ to $\Gat$ must satisfy
$\s(R(w,w')) \in \{0,1\}$ for every pair of labels $w$, $w'$.
To the best of our knowledge, the only other prover available for modal
fuzzy logics is mNiBLoS~\cite{Vidal:16}, an SMT-based solver designed
for continuous t-norm-based logics, including G\"odel--Dummett logic.
However, neither $\GWL$ nor $\GWCL$ are supported by mNiBLoS.

As future work, we plan to extend the procedure to other witnessed
G\"odel modal logics obtained by imposing additional constraints on the
