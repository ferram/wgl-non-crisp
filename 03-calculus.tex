...
\tr{Let $\M=\stru{W,R,e}$ be a $\GWM$-model;
...
\begin{proof}
  ...
  \tr{For the other cases see App.~\ref{app:proofs}.}
...
\tr{The relation $\precc$ is well-founded and can be used to instantiate
$\prec$ in the definition of strong termination (see
Lemma~\ref{lemma:rulesDec} in App.~\ref{app:proofs}); this proves that:}
...
% $O(\size{\varphi}^{\size{\varphi}})$. % (see Th.~4.1~\cite{BilkovaFK:22}).
...
% construction.  Nevertheless, using the above bounds, we can define a PSPACE
% algorithm to check if such a countermodel exists without effectively
% constructing it. The key observation is that, if $\varphi\not\in\GWL$,
% there is a countermodel where $R$ and $e$ take values in
% $\Qcal=\{0,\frac{1}{N},\frac{1}{N-1},\dots,1\}$, where $N\in
% O(\size{\varphi}^{\size{\varphi}})$. Indeed, using $\Qcal$, we can
% assign pairwise distinct values to $e(w,p)$ and $R(w,w')$, for every
% possible pair of worlds $w$, $w'$ and every propositional variable
% occurring in $\varphi$.
% %The key observation is that we can bound the number of values $N$
% %needed to define the maps $R$ and $e$ of the countermodel.  In
% %particular, if $\varphi\not\in\GWL$, there is a countermodel where $R$
% %and $e$ take values in $\Qcal=\{0,\frac{1}{N},\frac{1}{N-1},\dots,1\}$
% %where $N\in O(\size{\varphi}^{\size{\varphi}})$. Indeed, using
% %$\Qcal$, we can assign pairwise distinct values to $e(w,p)$ and
% %$R(w,w')$, for every possible pair of worlds $w$, $w'$ and every
% %propositional variable $p$ occurring in $\varphi$. 
% The algorithm iteratively tries to check whether a model such that
% $e(w_0,\varphi)=r_0$ exists, where $r_0\in\Qcal\setminus\{1\}$.
% %(where $w_0$ is a world of the model). 
% The algorithm maintains, for each generated world $w$, a set $\Phi(w)$
% of assignments of the form $e(w,\a)=r$, where $\a$ is a subformula of
% $\varphi$ and $r\in \Qcal$, specifying the intended value $r$ of
% $e(w,\a)$. We remark that, at each step of the execution, at most
% $O(\size{\varphi})$ entries of the map $\Phi$ must be recorded, and
% each entry $\Phi(w)$ requires $O(\size{\varphi})$ space.  The
% algorithm proceeds in two phases. First, the assignments $e(w,\a)=r$
% in $\Phi(w)$, where $\a$ is non-modal, are saturated by assigning
% values in $\Qcal$ to the subformulas of $\a$ and updating the
% assignments in $\Phi(w)$ accordingly.  At the end of this saturation
% phase, every formula $\a$ occurring in $\Phi(w)$ is either atomic or
% modal. If all the $\a$ are atomic, the procedure terminates
% successfully if the assignments in $\Phi(w)$ are coherent (i.e.,
% $\Phi(w)$ does not assign distinct values to the same formula),
% otherwise the procedure backtracks and tries different assignments
% from $\Qcal$. We point out that the memory space required to manage
% backtracking is $O(|\varphi|)$. Secondly, a loop is entered where, for
% every $e(w,\a)=r$ in $\Phi(w)$ such that $\a$ is modal, an
% $R$-successor world of $w'$ is generated (the witness for $\a$). At
% each iteration, a value from $\Qcal$ for $R(w,w')$ is chosen and
% $\Phi(w')$ is initialized so that it is consistent with the
% assignments in $\Phi(w)$ for modal formulas; then, the procedure is
% recursively applied to $w'$, relying on the assignments in
% $\Phi(w')$. Each recursive call strictly decreases the number of modal
% operators involved, hence the recursion depth is bounded by
% $|\varphi|$.  If all the required $R$-successors of $w$ can be
% generated, the procedure terminates successfully; accordingly, a
% $\GWM$-model such that $e(w_0,\varphi)=r_0$ can be built, hence
% $\varphi\not\in\GWL$.  Otherwise, the procedure backtracks.  If all
% the attempts to construct a countermodel fail, we conclude that
% $\varphi\in\GWL$.  One can show that the outlined procedure runs in
% PSPACE, hence validity in $\GWL$ can be decided in PSPACE; see
% App.~\ref{app:pspace} for further details.
