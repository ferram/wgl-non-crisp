...
backtracking is $O(|\varphi|)$. Secondly,
a loop is entered where, for every   $e(w,\a)=r$ in $\Phi(w)$ such that $\a$ is modal,  an $R$-successor world
of $w'$ is generated (the witness for $\a$). At each iteration, a value from $\Qcal$ for $R(w,w')$ is chosen
and $\Phi(w')$ is initialized so that it is consistent with the
...
$|\varphi|$.  If all the required $R$-successors of $w$ can be generated,
the procedure terminates successfully; accordingly, a $\GWM$-model
such that $e(w_0,\varphi)=r_0$ can be built, hence
...
PSPACE, hence validity in $\GWL$ can be decided in PSPACE; see App.~\ref{app:pspace} for further details.
