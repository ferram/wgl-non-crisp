...
we often replace a   cluster $\clust{z}$ with the $z$ (e.g.,
we write $\Rnof{S_{x,y}}$ or   $\Rnof{S_{\clust{x},y}}$  instead of
$\Rnof{S_{\clust{x},\clust{y}}}$).

% By definition of $\Rnof{S_{\clust{x},\clust{y}}}$ and property~\ref{F1} we get:

% \begin{enumerate}[label=(P\arabic*), ref=(P\arabic*),leftmargin=*,start=4]

% \item\label{prop:Rn:4}
%   Let $\Rnof{S_{\clust{x},\clust{y}}}=r$. For every $x'\in\clust{x}$ such that $\Rnof{x'}\leq r$,
%   there exists $y'\in\clust{y}$ such that $x' S y'$.
