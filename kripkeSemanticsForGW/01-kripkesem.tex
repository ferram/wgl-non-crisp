...
$x_1\forcing  \Box \a$.
\end{enumerate}
Let us assume, by contradiction,  $x_1\nforcing  \Box \a$. Firstly, we prove that:
\begin{enumerate}[label=(\alph*), ref=(\alph*),leftmargin=*.start=2]
\item\label{ex:axiomGW:2}
  Let  $\xmax$ be the $\leq$-maximum element in the cluster $\clust{x_1}$; then,
   $\xmax \forcing \Box \a$. 
\end{enumerate}
Let us assume $\xmax \nforcing \Box \a$. Then, there exists $z\in X$ such that
$\xmax S z$ and $z\nforcing \a$. Since $x_1\forcing\Box\psi$ and $x_1\leq \xmax$, it follows that $z\forcing \psi$.
Since  $z\nforcing \a$, we get $z\nforcing \b$, and this implies $z\nforcing \a \to \b$.
Note that $z$ is  the $\leq$-maximum element in the cluster $\clust{z}$,
hence $z\forcing \a$, a contradiction. This proves~\ref{ex:axiomGW:1}.
From~\ref{ex:axiomGW:1} and the assumption $x_1\nforcing  \Box \a$, we get:
\begin{enumerate}[label=(\alph*), ref=(\alph*),leftmargin=*,start=3]
\item\label{ex:axiomGW:3}
  there exists $x_2\in X$ such that $x_1 < x_2$ and   $x_2\forcing\Box\a$  and $(x_2)^-\nforcing \Box \a$.
\end{enumerate}
Since  $(x_2)^-\nforcing \Box \a$, there exists $y\in X$ such that
...
This yields a contradiction, since $x'S y'$ and $y'\nforcing \b$, and this proves~\ref{ex:axiomGW:1}
From the fact that   $x_1\forcing \Box \a\to \Box\b$ and~\ref{ex:axiomGW:1}, we get   $x_1\forcing\b$.
This conclude the proof that   $x_0\forcing \varphi_\Box$, and this implies
$\varphi_\Box\in\GWL$.
\EndEx
\end{example}
...
\begin{example}\label{ex:crispModel}
  ...
  $x_1\forcing  \Diam \b$.
 Since    $x_1\nforcing \Box \a$,   by Lemma~\ref{lemma:crispModel}\ref{lemma:crispModel:2}, 
...
Since $y_1\forcing \a$, we get $y\forcing \neg \a$;
it follows that  $y_0\forcing\neg\neg \a$,  hence  $x_0\forcing\Diam\neg\neg \a$.
This proves that $\varphi$ is valid in every   regular $\GWM$-bimodel; we conclude
 $\varphi\in\GWCL$.
\EndEx  
  \end{example}
...
we assume
$x_1\nforcing \Diam \g$ and we prove $x_1\forcing \Diam \psi$.
...
By~\ref{F5}, there exist $y_1$ and  $y'$ such that  $x_1 S y_1$ and  $(x_2)^- S y'$. Note that:
\begin{itemize}
\item
  $y_1\leq y'< y_2$ and $y'=(y_2)^-$.  
\end{itemize}
Note that   $(x_2)^-\nforcing \Diam \a$ (otherwise,  $(x_2)^-\forcing \Diam \b$ would follow), hence $y'\nforcing \a$ and  $y'\nforcing \b$.
Since $y_2\forcing \b$, we get $y_1\forcing \psi$, and this proves $x_1\forcing \Diam \psi$.
This concludes the proof of  $x_0\forcing \varphi_\Diam$.
We conclude $\varphi_\Diam\in\GWCL$. \EndEx
 \end{example} 
