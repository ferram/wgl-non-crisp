...
order relation (intuitionistic accessibility relation), $S\subseteq X \times X$ (modal accessibility relation),
...
\end{itemize}
The  following Fischer-Servi conditions must be met (see Fig.~\ref{fig:F}):
...
\begin{figure}[t]
  ...
  \caption{Conditions~\ref{F1}--\ref{F4}.}\label{fig:F}
\end{figure}
...
a correspondence between the class of discrete $\GWM$-models and a proper class
of $\GWM$-bimodels which preserves validity of formulas.
...
\noindent
Let $\K=\stru{X,\leq,S,V}$ be a regular model.
The \emph{height} of $\K$, denoted by $\height{\K}$, is the height of a cluster of $\K$.
...
\Rnof{\clust{x},\varphi} \;=\; \max\left(\,\{\,
  ...
  \,\}\,\cup\,\{0\}\,\right)
\]
Note that $\Rnof{\clust{x},\varphi} \in \Rnof{X}\cup\{0\}$, where $\Rnof{X}$
is the image of $\Rn$.
...
\item\label{prop:Rn:1}
  $\Rnof{\clust{x},\varphi}=0$  iff $\xmax\nforcing \varphi$;
\item\label{prop:Rn:2}
  $\Rnof{\clust{x},\varphi}=1$ iff $\xmin\forcing \varphi$; 


\item\label{prop:Rn:3}
  Let $\Rnof{\clust{x},\varphi}=r$, where $0 < r < 1$, and let $x_r\in \clust{x}$ be
...
denoted by $\Rnof{S_{\clust{x},\clust{y}}}$   is defined as follows:
\[
...
\right)
\]
In the notations  $\Rnof{\clust{x},\varphi}$ and $\Rnof{S_{\clust{x},\clust{y}}}$,
we often replace a   cluster $\clust{z}$ with an element $z$;
for instance, 
we write $\Rnof{S_{x,y}}$   instead of $\Rnof{S_{\clust{x},\clust{y}}}$).
...
\[
 ...
 \\[1ex]
      \Rnof{y_1,p\land q} =0.4,\quad   \Rnof{y_1,p\lor q} =1,\quad  \Rnof{z_1,q\to p} =0.6\quad \Rnof{z_1,p\to q} = 1,
\\[.5ex]
...
The intuitionistic accessibility relation $\leq$ is
  represented by the straight lines ($w\leq w'$ iff $w$ is below or
  equal to $w'$), the modal accessibility relation $S$ is depicted by  arrows.
...
\begin{lemma}\label{lemma:Rn}
  Let $(\K,\Rn)$ be a ranked   $\GWM$-bimodel, where $\K=\stru{X,\leq,S,V}$.
  \begin{enumerate}[label=(\roman*), ref=(\roman*),leftmargin=*]
...
\end{lemma}
\begin{proof}
 \emph{Proof of \ref{lemma:Rn:prop}}.
...
\smallskip
\noindent
\emph{Proof of \ref{lemma:Rn:box}}.
Let $r=\Rnof{x,\Box \a}$;   we show that:

 \begin{enumerate}[label=(\arabic*), ref=(\arabic*),leftmargin=*]
\item\label{lemma:Rn:box:goal}
  $r\,= \,\min_{y\in X}\{\,\Rnof{S_{x,y}}\to \Rnof{y,\a} \,\}$.
\end{enumerate}

\noindent 
Let us assume $r=0$. By~\ref{prop:Rn:1}, it holds that $\xmax \nforcing  \Box \a$, hence
there exists $y^\star\in X$ such that $\xmax S y^\star$ and $y^\star\nforcing \a$.
By Lemma~\ref{lemma:reg}\ref{lemma:reg:1}, $\height{y^\star}=\height{\xmax}=0$;
it follows that  $y^\star=\ymax$, hence
$\Rnof{y^\star,\a}=0$.
...
By~\ref{F1}, there exists $y_r\in\clust{y}$  such that  $x_r S y_r$
and $y \leq y_r$; by Lemma~\ref{lemma:reg}\ref{lemma:reg:1},
$\Rnof{y_r} = \Rnof{x_r}$, namely
$\Rnof{y_r} = r$.
We prove that:
...
By Lemma~\ref{lemma:reg}\ref{lemma:reg:1} $\Rnof{y_1} = \Rnof{x_1}$,
hence  $\Rnof{y_1} = \Rnof{S_{x,y}}$; since we are assuming   $\Rnof{S_{x,y}} >  \Rnof{y,\a}$,
we get $\Rnof{y_1} > \Rnof{y,\a}$.
It follows that  $y_1\nforcing\a$, hence $x_1\nforcing \Box\a$.
Accordingly,
$\Rnof{x_1}> r$, and this implies $x_1 < x_r$. 
Since $\Rnof{S_{x,y}} =\Rnof{x_1}$,
there exists $y_1\in \clust{y}$ such that $x_1 S y_1$.
By~\ref{F1},  there exists
 $y_r$ such that $x_r S y_r$ and $y_1\leq y_r$;  by Lemma~\ref{lemma:reg}\ref{lemma:reg:1}, $\Rnof{y_r} =\Rnof{x_ r}= r$. 
...
From~\ref{lemma:Rn:box:2} and~\ref{lemma:Rn:box:3}, point~\ref{lemma:Rn:box:goal} follows.
...
\smallskip
\noindent
\emph{Proof of \ref{lemma:Rn:diam}}.
Let $r=\Rnof{x,\Diam \a}$;   we show that:

 \begin{enumerate}[label=(\arabic*), ref=(\arabic*),leftmargin=*,start=2]
\item\label{lemma:Rn:diam:goal}
  $r\,=\,\max_{y\in X}\{\,\Rnof{S_{x,y}}\land \Rnof{y,\a} \,\}$.
\end{enumerate}
Let us assume $r=0$. By~\ref{prop:Rn:1}, it holds that $\xmax \nforcing  \Diam \a$.
We show that:
...
By Lemma~\ref{lemma:reg}\ref{lemma:reg:1},   $\height{y^\star}= \height{\xmax} = 0$; as a consequence,    $y^\star = \ymax$ and $\Rnof{y,\a}=0$.
...
by Lemma~\ref{lemma:reg}\ref{lemma:reg:1},
 $\Rnof{y_r}=  \Rnof{x_r} = r$, hence $\Rnof{y_r,\a}\geq r$.
We prove that:
...
Let us assume   $\Rnof{S_{x,{y_r}}}> r$.  Note that  $\Rnof{S_{x,{y_r}}}\geq  \Rnof{(x_r)^-}$,
hence there exists $y^\star\in \clust{y_r}$ such
  that $(x_r)^- S y^\star$.
  Since $(x_r)^-\nforcing \Diam\a$, we get  $y^\star\nforcing \a$.
...
We show that
 ...
 Note that  $\Rnof{S_{x,y}}\geq  \Rnof{(x_r)^-}$,
 hence  there exists $y^\star\in \clust{y}$ such that  $(x_r)^- S y^\star$.
 By Lemma~\ref{lemma:reg}\ref{lemma:reg:1}  $\Rnof{y^\star}= \Rnof{(x_r)^-}$.
 Since $\Rnof{x,\Diam \a}=r$ and $\Rnof{(x_r)^-} > r$, we have $(x_r)^-\nforcing \Diam \a$,
 hence  $y^\star\nforcing \a$.
 It follows that  $\Rnof{y,\a}< \Rnof{y^\star}$, and this implies   $\Rnof{y,\a}\leq r$.
 It follows that  $\Rnof{S_{x,y}}\land \Rnof{y,\a}\leq  r$,
and this proves~\ref{lemma:Rn:diam:3}. From~\ref{lemma:Rn:diam:2} and~\ref{lemma:Rn:diam:3}, point~\ref{lemma:Rn:diam:goal} follows.
...
\noindent
A \emph{correspondence} between a  $\GWM$-model $\M=\stru{W,R,e}$ and a ranked
$\GWM$-bimodel $(\K,\Rn)$, where $\K=\stru{X,\leq,S,V}$, is a 1-1 map
...
between the  $\GWM$-model $\M=\stru{W,R,e}$  shown on  the left and 
the ranked $\GWM$-bimodel $(\K,\Rn)$  represented on the right  (see Ex.~\ref{ex:rankedBiModel}).
 ...
 it holds that  $e(w,\varphi)=\Rnof{\Phi(w),\varphi}$; this crucial property is proved in the next proposition. 
   \EndEx  
 \end{example}
...
\begin{proposition}\label{prop:Phi}
  Let $\Phi$ be a correspondence between the $\GWM$-model $\M$ and  the ranked $\GWM$-bimodel $(\K,\Rn)$.
  For every world $w$ of $\M$
and every   formula $\varphi$,
...
\begin{proof}
Let $w$ be a world of $\M$  and $\varphi$ a formula; we prove  $e(w,\varphi) =\Rnof{\Phi(w),\varphi}$
  by induction on the size of $\varphi$. If $\varphi\in\PV$,
   ...
   Let $\varphi=\a\star \b$, where $\star\in \{\,\land,\,\lor,\,\to\,\}$. Then:
   \[
     \begin{array}{lcll}
...
& \text{induction hypothesis} 
       \\[1ex]                                                      
                        &  \;= \; &  \Rnof{\Phi(w),\a\star \b}
           & \text{Lemma ~\ref{lemma:Rn}\ref{lemma:Rn:prop}}.
        \end{array}    
   \]
...
\[
  \begin{array}{rcll}
  e(w,\Box \a) & \;=\; & \min\; \{\; R(w,w') \to e(w',\a)~|~w'\in W \;\}    
&
    \\[.5ex]
 & \;=\; & \min\;\{\; \Rnof{S_{\Phi(w),\Phi(w')} } \to e(w',\a)~|~w'\in W \;\}    
&\text{\ref{corr:1}}
\\[.5ex]
 & \;=\; &
...
Let $q_1\in \Qcal_1$; then:
\[
q_1\;=\;
  \Rnof{S_{\Phi(w),\Phi(w')} } \to \Rnof{\Phi(w'),\a}    
  \;=\;
  \Rnof{S_{\Phi(w),y}}\to  \Rnof{y,\a}
\;\in\Qcal_2  
\qquad
\begin{minipage}{5em}
    $w'\in W$
...
\end{minipage}
\]  
This proves $\Qcal_1\subseteq \Qcal_2$.
Conversely, let $q_2\in\Qcal_2$; then:
\[
q_2\;=\;   \Rnof{S_{\Phi(w),y}}\to  \Rnof{y,\a} 
\;=\;
...
% \]
% This proves $\Qcal_1\subseteq \Qcal_2$.
% Conversely, let $q_2\in\Qcal_2$
% and let us assume that $q_2=  \Rnof{S_{\Phi(w),y}}\to  \Rnof{y,\a}$, where $y\in X$.
% Since $\Phi$ is surjective, there exists $w'\in W$ such that $\Phi(w') = \clust{y}$, hence:
% \[
% q_2\;=\;   \Rnof{S_{\Phi(w),y}}\to  \Rnof{y,\a} 
% \;=\;
%   \Rnof{S_{\Phi(w),\Phi(w')} } \to \Rnof{\Phi(w'),\a}  \;\in\Qcal_1
...
% Having proved that $\Qcal_1=\Qcal_2$, we conclude
%  $e(w,\Box \a)  = \Rnof{\Phi(w),\Box \a}$. 
 The proof that  $e(w,\Diam \a)  = \Rnof{\Phi(w),\Diam \a}$ is similar.
\end{proof}
...
\begin{proof}
  \ref{lemma:Phi:MtoK}. Let $\M=\stru{W,R,e}$ and let
  $\Qcal = (R(W)\cup e(W) \cup \{1\}) \setminus \{0\}$; since $\M$ is discrete, $\Qcal$ is a
finite subset of $(0,1]_Q$.  The $\GWM$-bimodel $\K=\stru{X,\leq,S,V}$
is defined as follows:
\[
  \begin{array}{ll}
    X\;=\;\{\, w_r~|~ \text{$w\in W$ and $r\in \Qcal$}\,\}
&    
\leq \;=\; \{\, (w_r, w'_{r'})~|~\text{$w = w'$ and $r \geq r'$ }  \,\}    
\\[1ex]
...
\end{array}
\]
    One can easily  check that $\K$ is a  well-defined regular $\GWM$-bimodel.
    For every $w\in W$, the set of  $w_r$ such that $r\in \Qcal$ is a cluster of $\K$.
    We point out that, if  $(w_r, w'_r)\in S$, then $R(w,w') > 0$.
    Let  $r_0,r_1, \dots, r_{k}$ be an enumeration of the elements of $\Qcal$ in ascending order
    (thus, $r_k=1$).
   The map $\Rn : \{0,\dots,k\}\to\Qcal$
    such that $\Rnof{k} = r_k$ is a ranking over $\K$;  note that $\Rnof{w_{r_j}}=r_j$.
    The function $\Phi$ mapping a world $w$ of $\M$ to the cluster
...
W\,=\,\{\, \clust{x}~|~x\in W   \,\}
       \qquad
       R(\clust{x},  \clust{y}) \,=\, \Rnof{S_{x,y}}
\qquad
e(\clust{x},p)\,=\, \Rnof{\clust{x},p}
     \end{array}
\]
One can easily check that $\M$ is well-defined.  Let $\Phi$ be the identity map
between the worlds of $\M$ and the clusters of $\K$ 
(namely, $\Phi(\clust{x})=\clust{x}$, for every $x\in X$).
...
\\[1ex]
 $\Phi(x) = \clust{x_1}\qquad  \Phi(y) = \clust{y_1}$
   \[\small
    \begin{array}{rclcl}
      e(x,\Box (p\lor q))&=& 1 &=& \Rnof{\Phi(x),\Box(p\lor q)}
      \\[1ex]
      e(x,\Box p) &=&  0.5&=& \Rnof{\Phi(x),\Box p)}
      \\[1ex]
      e(x,\Diam q)&=&  0.6 &=& \Rnof{\Phi(x),\Diam q}
      \\[1ex]
      e(x,\chi) &=& 0.6  &=& \Rnof{\Phi(x),\chi}
    \end{array}
  \]
  
  \caption{A $\GWM$-countermodel for the formula $\chi=\Box (p\lor q)\to
...
\end{figure}
 ...
 \begin{example}\label{ex:countermodelCrisp}
Let us consider the $\GWM$-countermodel $\M$ for the formula $\chi=\Box (p\lor q)\to (\Box p \lor \Diam q) $ (instance of (Cr)) 
...
Since $e(x,\chi) = 0.6$, by Prop.~\ref{prop:Phi}
we get  $\Rnof{\clust{x_1},\varphi}=0.6$. Since $\Rnof{x_1}=1$, it holds that
$x_1\nforcing \chi$; accordingly, $\K$ is a countermodel for $\chi$.
 \end{example}



\begin{proposition}\label{prop:equivGW}
  $\LBMR =\GWL$.
\end{proposition}
...
e(w,\varphi)$, hence $\Rnof{\Phi(w),\varphi} < 1$.
Let $x^\star$  be the $\leq$-minimum element in the cluster $\Phi(w)$;
by  property~\ref{prop:Rn:2},   $x^\star\nforcing \varphi$,
hence  $\varphi\not\in \LBMR$. This proves that $\LBM\subseteq \GWL$.  The
proof of $\GWL\subseteq \LBMR$ is similar and relies on
...
\begin{example}\label{ex:axiomGW}
  Let $\varphi_\Box$ be the axiom 
  \[
\Box(\, \underbrace{((\a\to \b)\to\b) \,\land\, (\b\to\a)}_{\psi}  \,) \; \to \; (\,(\Box\a \to \Box\b) \to \Box \b\,)
  \]
introduced in~\cite{VidalR:26}.
  We exploit Prop.~\ref{prop:equivGW} to show that $\varphi_\Box\in\GWL$.
  Let  $\K=\stru{X,\leq,S,V}$ be a regular   $\GWM$-bimodel and let  $x_0\in X$;
  we show that  $x_0\forcing \varphi_\Box$.
  Let $x_1\in X$ such that
\begin{itemize}
\item $x_0\leq x_1$ and $x_1\forcing \Box \psi$ and $x_1\forcing \Box \a\to \Box\b$;
\end{itemize}
we prove that
\begin{enumerate}[label=(\alph*), ref=(\alph*),leftmargin=*]
\item\label{ex:axiomGW:1}
$x_1\forcing  \Box \b$.
\end{enumerate}
To prove~\ref{ex:axiomGW:1}, we show that $x_1\forcing  \Box \a$.
Let us assume, by contradiction,  $x_1\nforcing  \Box \a$;
let  $\xmax$ be the $\leq$-maximum element in the cluster $\clust{x_1}$.
Firstly, we show that  $\xmax \forcing \Box \a$.
Let  $z\in X$ such that $\xmax S z$. Since  $x_1\forcing\Box\psi$ and $x_1\leq \xmax$,
it holds that $z\forcing \psi$.  Note that $z$ is  the $\leq$-maximum element in the cluster $\clust{z}$,
hence $z\forcing \a$; this proves  $\xmax \forcing \Box \a$.
Since   $x_1\nforcing  \Box \a$  and   $\xmax \forcing \Box \a$ and $x_1\leq \xmax$,   the following property holds:
\begin{enumerate}[label=(\alph*), ref=(\alph*),leftmargin=*,start=2]
\item\label{ex:axiomGW:2}
  there exists $x_2\in X$ such that $x_1 < x_2$ and   $x_2\forcing\Box\a$  and $(x_2)^-\nforcing \Box \a$.
\end{enumerate}
Since  $(x_2)^-\nforcing \Box \a$ and  $x_2\forcing\Box\a$, there exists $y\in X$ such that
$(x_2)^- S y$ and $y\nforcing \a$. Note that $x_1\leq (x_2)^-$, hence $y\forcing \psi$.
It follows that  $y\nforcing\b$,  which implies $y\nforcing \a\to \b$.
There exists $y'\in X$ such that  $y \leq y'$ and   $y'\forcing \a$ and $y'\nforcing \b$.
By~\ref{F2}, there exists $x'\in X$ such that $(x_2)^-\leq x'$ and $x' S y'$.
We remark that $y <  y'$ (indeed,  $y\nforcing \a$  and $y'\forcing \a$ ), hence   $(x_2)^-< x'$, which implies  $x_2\leq x'$.
Since $x_1\forcing \Box\a\to \Box\b$ and $x_2\forcing \Box\a$ and  $x_1< x_2\leq x'$, we get $x'\forcing\Box \b$.
This yields a contradiction, since $x'S y'$ and $y'\nforcing \b$;
as a consequence,  the assumption   $x_1\nforcing  \Box \a$ is false,
thus   $x_1\forcing  \Box \a$.  Since  $x_1\forcing \Box \a\to \Box\b$, we get $x_1\forcing  \Box \b$,
and this proves~\ref{ex:axiomGW:1}.
We conclude  that   $x_0\forcing \varphi_\Box$, accordingly
$\varphi_\Box\in\GWL$.
\EndEx
\end{example}
...
Let $\K=\stru{X,\leq,S,V}$ be a regular $\GWL$-bimodel; $\K$ is \emph{crisp} iff the following condition holds:

\begin{enumerate}[label=(F\arabic*), ref=(F\arabic*),leftmargin=*,start=5]
\item\label{F5} 
 if  $x_1\leq x_2$ and  $x_2 S y_2$,  there exists $y_1$ such that  $x_1 S y_1$ and  $y_1\leq y_2$.
 ...
 % F5
    ...
    \begin{tikzpicture}[scale=0.8]
      ...
      \draw (-0.5,0.5) node{$\leq$}; 
      \draw (1,-0.3) node{$S$}; 
      \draw (1,0.5) node{\ref{F5}}; 
           
    \end{tikzpicture}
 
\end{enumerate}
Note that none of the $\GWL$-models in Figs.~\ref{fig:rankedBiModel} and \ref{fig:corrCrispAx} is crisp.
For instance, in the model in  Fig.~\ref{fig:rankedBiModel} it holds that $x_1\leq x_{0.8}$ and $x_{0.8} S y_{0.8}$,
but there is no $y$ such that  $x_1 S y$ and $y\leq y_{0.8}$. 

\begin{lemma}\label{lemma:crispModel}
  Let $\K=\stru{X,\leq,S,V}$ be a crisp regular $\GWL$-bimodel and let  $x\in X$.

   \begin{enumerate}[label=(\roman*), ref=(\roman*),leftmargin=*]
   \item\label{lemma:crispModel:1}
Assume $x Sy$.  Then,
 for every $x'\in\clust{x}$ there exists  $y'\in\clust{y}$ such that $x'S y'$.


\item\label{lemma:crispModel:2} 
Let  $\Rn$ be a ranking over $\K$.  For every $x,y\in X$,  $\Rnof{S_{x,y}}\in\{0,1\}$.

\item\label{lemma:crispModel:3} 
$x\forcing \Box \a$ if and only if, for every $y\in X$ such that $x S y$, $y\forcing \a$.
...
\begin{example}\label{ex:crispModel}
  In Ex.~\ref{ex:countermodelCrisp} we have shown a regular $\GWL$-model falsifying an instance of
  the Crisp axiom $(Cr)=\Box (\a\lor \b)\to (\Box \a \lor \Diam \b)$. Note that such a model is not crisp.
  We show that $(Cr)$ is valid in all crisp
  regular $\GWL$-bimodels.
  Let  $\K=\stru{X,\leq,S,V}$ be a crisp regular  $\GWL$-bimodel and let $x_0\in X$,
  we show that $x_0\forcing (Cr)$.
  Let  $x_1\in X$ such that $x_0\leq x_1$ and  $x_1\forcing \Box (\a\lor \b)$, we show that
  $x_1\forcing \Box \a \lor \Diam \b$. To this aim, we assume    $x_1\nforcing \Box \a$ and we prove
  $x_1\forcing  \Diam \b$.
 Since    $x_1\nforcing \Box \a$,   by Lemma~\ref{lemma:crispModel}\ref{lemma:crispModel:3}, 
  there exists $y_1\in X$ such that  $x_1 S y_1$ and    $y_1\nforcing  \a$. By the fact that  $x_1\forcing \Box (\a\lor \b)$,
  we get $y_1\forcing \a\lor \b$; this implies $y_1\forcing \b$,
  hence  $x_1\forcing  \Diam \b$. This proves that $x_1\forcing \Box \a \lor \Diam \b$.
  We conclude $x_0\forcing \psi$, hence  $(Cr)$ is valid in all crisp  regular $\GWL$-bimodels.
\EndEx
\end{example}



\begin{proposition}\label{prop:PhiCrisp}
 Let $\Phi$ be a correspondence between the $\GWM$-model $\M$ and  the ranked $\GWM$-bimodel $(\K,\Rn)$.
Then, $\M$ is crisp if and only if $\K$ is crisp.
\end{proposition}
...
\begin{proposition}\label{prop:equivGWC}
  The set of formulas valid in all the crisp  regular $\GWM$-bimodel coincides with $\GWCL$.
\end{proposition}

\begin{example}\label{ex:axiomGWC1}
Let $\varphi=\neg \Diam \a\lor \Diam\neg\neg \a$.   We exploit Prop.~\ref{prop:equivGWC} to show that $\varphi\in\GWCL$.
Let $\K=\stru{X,\leq,S,V}$ be a  crisp regular  $\GWM$-bimodel and let $x_0\in X$;
we show $x_0\forcing \varphi$.
To this aim, we assume  $x_0\nforcing\neg \Diam \a$ and we prove   $x_0\forcing\Diam\neg\neg \a$.
Since   $x_0\nforcing\neg \Diam \a$, there exists $x_1\in X$ such that $x_0\leq x_1$ and $x_1\forcing  \Diam \a$;
hence  there exists $y_1\in X$ such that $x_1 S y_1$ and $y_1\forcing \a$.
By~\ref{F5}, there exists $y_0\in X$ such that  $x_0 S y_0$ and $y_0 \leq y_1$.
Since $y_1\forcing \a$, 
it follows that  $y_0\forcing\neg\neg \a$,  hence  $x_0\forcing\Diam\neg\neg \a$.
This proves that    $x_0\forcing \varphi$, and this implies
 $\varphi\in\GWCL$.
\EndEx  
  \end{example}

  
\begin{example}\label{ex:axiomGWC2}

Let $\varphi_\Diam$ be the axiom 
  \[
    (\,(\Diam\a\to\Diam\b)\,\land\,((\Diam\b\to\Diam\g)\to\Diam\g)\,)\;\to\;
    (\, \Diam\g\,\lor\,\Diam(\,\underbrace{(\a\to\b)\land((\b\to\g)\to\g)}_\psi \,)   \,)
  \]
introduced in~\cite{VidalR:26}.
Let  $\K=\stru{X,\leq,S,V}$ be  a crisp regular  $\GWM$-bimodel and let $x_0\in X$;
we show that $x_0\forcing \varphi_\Diam$.
Let $x_1\in X$ such that:
\begin{itemize}
\item $x_0\leq x_1$ and  $x_1\forcing   \Diam\a\to\Diam\b$ and   $x_1\forcing (\Diam\b\to\Diam\g)\to\Diam\g$
\end{itemize}
We prove:
\begin{enumerate}[label=(\alph*), ref=(\alph*),leftmargin=*]
\item\label{ex:axiomGWC:1}
$x_1\forcing \Diam \g\lor\Diam \psi$.  
\end{enumerate}
Let us assume  $x_1\forcing \Diam\b$.
Then, there exists $y_1\in X$ such that $x_1 S y_1$ and $y_1\forcing \b$;
it follows that $x_1\forcing \Diam \psi$, hence~\ref{ex:axiomGWC:1} holds.

Let us assume  $x_1\nforcing \Diam\b$; to prove~\ref{ex:axiomGWC:1},
we assume
$x_1\nforcing \Diam \g$ and we show $x_1\forcing \Diam \psi$.
Since $x_1\nforcing \Diam \g$, we get   $x_1\nforcing \Diam\b\to\Diam\g$.
There exists $x'\in X$ such that $x_1\leq x'$ and $x'\forcing  \Diam\b$ and
$x'\nforcing \Diam\g$. Since  $x_1\nforcing\Diam \b$, there exists $x_2\in X$ such that:
\begin{itemize}
\item $x_1 < x_2$ and  $x_2\forcing \Diam\b$ and $(x_2)^-\nforcing \Diam \b$.
\end{itemize}
There exists $y_2\in X$ such that $x_2 S y_2$ and $y_2\forcing\b$. 
By~\ref{F5}, there exist   $y'$ and $y_1$ such that:
\begin{itemize}
\item $(x_2)^- S y'$ and $y'\leq y_2$;

\item $x_1 S y_1$ and $y_1\leq y_2$.
\end{itemize}
Since $x_1\leq (x_2)^- < x_2$, we get:
\begin{itemize}
\item
  $y_1\leq y'< y_2$ and $y'=(y_2)^-$.  
\end{itemize}
Note that  $(x_2)^-\nforcing \Diam \b$  and   $(x_2)^-\nforcing \Diam \a$ (otherwise,  $(x_2)^-\forcing \Diam \b$ would follow),
hence $y'\nforcing \b$ and  $y'\nforcing \a$.
Since $y_2\forcing \b$, we get $y_1\forcing \psi$, hence $x_1\forcing \Diam \psi$,
and this concludes the proof of~\ref{ex:axiomGWC:1}.
We  conclude  $x_0\forcing \varphi_\Diam$,
hence $\varphi_\Diam\in\GWCL$. \EndEx
 \end{example} 
...
x_2$, $x_1 S y_1$,  $V(x_1)=V(x_2)=\emptyset$,   $V(y_1)=\{p\}$; $\K$ does not match
...
In the same way, if  a crisp $\GWL$-bimodel does not satisfy condition~\ref{F5},
we can falsify the instance $\neg\Diam p\lor \Diam\neg\neg p$ of the formula in  Ex.~\ref{ex:axiomGWC1}
which is valid in $\GWCL$:
...
$\GWL$ violating condition~\ref{F4};
