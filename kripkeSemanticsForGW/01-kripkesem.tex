...
Let us assume, by contradiction,   $\varphi\not\in\GWL$.
By  Prop.~\ref{prop:equivGW}, there exists a regular  $\GWM$-bimodel $\K=\stru{X,\leq,S,V}$
and $x_0\in X$ such that $x_0\nforcing \varphi$.
It follows that there exists $x_1$  such that:
\begin{itemize}
\item $x_0\leq x_1$;

\item $x_1\forcing \Box \psi$ and $x_1\forcing \Box \a\to \Box\b$ and $x_1\nforcing \Box \b$.
\end{itemize}
Note that  $x_1\nforcing \Box \a$.  We prove that:
...
Since $x_1\forcing \Box\a\to \Box\b$ and $x_2\forcing \Box\a$ and  $x_1< x_2\leq x_1$, we get $x'\forcing\Box \b$.
This yields a contradiction, since $x'S y'$ and $y'\nforcing \b$.
We conclude that $\varphi_\Box\in\GWL$.
\EndEx
\end{example}
...
By~\ref{F5}, there exist $y_1$ and  $yX$ such that  $x_1 S y_1$ and  $(x_2)^- S y'$. Note that:
\begin{itemize}
\item
