...
\item $x\forcing \a \to \b$ iff for every $y\in X$ s.t.  $y\geq x$, if
  $y \forcing \a$ then $y \forcing \b$;
...
\varphi$ and $x\leq y$, then $y\forcing\varphi$; in particular,
...
a correspondence between the class of discrete \GWM-models and the
...
\begin{itemize}
\item $\xmin$  denotes the $\leq$-minimum element in $\clust{x}$;
\item  $\xmax$ the $\leq$-maximum element in $\clust{x}$;

\item  If $x$ is not the $\leq$-minimum in $\clust{x}$,  $x^-$  denotes the immediate $\leq$-predecessor
  of $x$.
...
\[
  \height{x}\;=\;
  \begin{cases}
0 &\text{if $x$ is the $\leq$-maximum of $\clust{x}$}    
\\
...
Let  $\height{\K}=k$;  a \emph{ranking} over $\K=\stru{X,\leq,S,V}$ is an order preserving map 
 $\Rn : \{0,\dots, k\} \to (0,1]_\mathrm{Q}$  such that:
 \begin{itemize}
 \item  if $i < j$, then   $\Rnof{i} < \Rnof{j}$. 
...
\noindent
A \emph{ranked $\GWM$-bimodel} is a pair $(\K,\Rn)$, where $\K$ is a
$\GWM$-bimodel and $\Rn$ is a ranking over $\K$.
Let $(\K,\Rn)$ be a ranked $\GWM$-bimodel.
By Lemma~\ref{lemma:reg}\ref{lemma:reg:1}, if  $ x S y$, then $\height{x}=\height{y}$.
...
\\[.5ex]
 ...
 \\[1ex]
  ...
  \Rnof{x_1,\Diam p} =0.8,\quad \Rnof{x_1,\Diam q} = 0.6
    \end{array}
\]
  \caption{A ranked $\GWM$-bimodel $(\K,\Rn)$.}
...
displayed in Fig.~\ref{fig:rankedBiModel}, where:
\[
   \begin{array}{l}
...
\leq\;=\;\{\,(w_r,w'_{r'})~|~\text{$w=w'$ and $r\geq r'$} \,\}
     \\[1ex]
     S\;=\;\{\, (x_r,y_r)~|~x\leq 0.8 \,\}\;\cup\; \{\, (x_r,z_r)~|~x\leq 0.6 \,\}
     \\[1ex]
     V(w)\;=\;
     \begin{cases}
...
\\[1ex]
 \emptyset   & \text{otherwise}         
   ...
   \end{array}
\]
In the figure, the relation $\leq$ is
 ...
 The   propositional variables in $V(w_r)$ are displayed to the right of
  $w_r$.  There are three clusters $\clust{x_1}$, $\clust{y_1}$ and
