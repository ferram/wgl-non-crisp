...
and the rankimg  $\Rn$ over $\K$ are defined as follows:
\[
  \begin{array}{ll}
    X\;=\;\{\, w_r~|~ r\in \Qcal\,\}
&    
...
\end{array}
\]
    One can easily  check that $\K$ is a  well-defined regular bimodel.
   For every $w\in W$, the set of  $w_r$ such that $r\in \Qcal$ is a cluster of $\K$.
    Let  $q_0,r_1, \dots, q_{k}$ be an enumeration of the elements of $\Qcal$ in ascending order
    (thus, $q_k=1$).
   The map $\Rn : \{0,\dots,k\}\to\Qcal$
    such that $\Rnof{k} = q_k$ is a ranking over $\K$.  Note that $\Rnof{w_{q_j}}=q_j$.
