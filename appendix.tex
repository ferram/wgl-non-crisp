...
\inlinetodo{** OLD PROOF **}
  ...
  Let $\Ical$ be an $\M$-interpretation, let $w$ be a label of $\LC$ and
$w^\star$ a world of $\M$.  By $\reass{\Ical}{w}{w^\star}$ we denote
the $\M$-interpretation $\Ical'$ such that $\Ical'(w) = w^\star$ and
$\Ical'(x)= \Ical(x)$ for every $x\neq w$ (where $x$ is either a label
or a constant).  The definition of $\reass{\Ical}{c}{r}$, where $c$ is
a constant of $\LC$ and $r\in [0,1]_Q$, is similar.
...
\[
%% IMP LTS 
\AXC{$\overbrace{w :\b \leq b,\, w :\a > b,\,  b  < t,\,  \G_0}^{\G_1}$}    
% ------------------------------------------    
\RightLabel{$\to < $}
...
t$.  If $\b\in\PV\cup\{\bot\}$, then $b$ stands for $w:\b$, hence
$\M\models_\Ical \G_1$.  Let us assume $\b\not\in\PV\cup\{\bot\}$; in
this case $b$ is a fresh constant.  Let $r = e(\Ical(w),\b)$
($r\in[0,1]_Q$) and $\Ical'=\reass{\Ical}{b}{r}$.  We remark that, for
every constraint $\chi$ not containing $b$, $\M\models_\Ical \chi$ iff
$\M\models_{\Ical'} \chi$.  Since $\M\models_{\Ical} \G_0$ and $b$
does not occur in $\G_0$, we get $\M\models_{\Ical'} \G_0$.  It is
easy to check that $\M\models_{\Ical'} w :\b \leq b$ and
$\M\models_{\Ical'} w :\a > b$ and $\M\models_{\Ical'} b < t$, hence
$\M\models_{\Ical'} \G_1$.
...
\AXC{$\overbrace{w :\b \leq b,\,  w :\a > b,\,  b \leq  t,\,\G_0}^{\G_2}$}    
% ------------------------------------------    
\RightLabel{$\to\leq$}
...
hence $\M\models_\Ical w:\b \leq t$.  If $\b\in\PV\cup\{\bot\}$, then
$b= w:\b$ and we get $\M\models_\Ical \G_2$.  Assume
$\b\not\in\PV\cup\{\bot\}$ (thus $b$ is a fresh constant), let $r =
e(\Ical(w),\b)$ ($r\in[0,1]_Q$) and $\Ical'=\reass{\Ical}{b}{r}$.
Since $\M\models_{\Ical} \G_0$ and $b$ does not occur in $\G_0$, we
get $\M\models_{\Ical'} \G_0$.  Moreover, one can easily check that
$\M\models_{\Ical'} w :\b \leq b$ and $\M\models_{\Ical'} w :\a > b$
and $\M\models_{\Ical'} b \leq t$; we conclude $\M\models_{\Ical'}
\G_2$.
...
\[
%% IMP GT 
\AXC{$\overbrace{w :\a \leq a,\,  w :\b \geq a,\,1\gt t,\, \G_0}^{\G_1}$}    
...
$\M\models_{\Ical'} \G_1$, where $\Ical'=\Ical$ if
$\a\in\PV\cup\{\bot\}$, $\Ical'= \reass{\Ical}{a}{e(\Ical(w), \a)}$
otherwise.

Let us consider the application
\[
\AXC{$\overbrace{1 \lt t,\,\Phizo{\G_0}}^{\G_1}$}
\AXC{$\overbrace{w_1:\a \lt t,\,\Phibd{\G,w,w_1},\,\G_0}^{\G_2}$}
% -----------------------------------------
\RightLabel{$\Box\lt$}
\BIC{$\underbrace{w:\Box\a \lt t,\,\G_0}_{\G}$}    
\DP
\]
By the hypothesis, $\M\models_{\Ical} w:\Box\a \lt t$ and
$\M\models_{\Ical} \G_0$, hence $e(\Ical(w),\Box \a)\lt \Ical(t)$.
Let us assume that the world $\Ical(w)$ of $\M$ has no $R$-successors.
For every formula $\varphi$, $e(\Ical(w),\Box\varphi) = 1$ and
$e(\Ical(w),\Diam\varphi) = 0$, and this implies that $\M\models_\Ical
\G_1$.  Let us assume that $\Ical(w)$ has at least one
$R$-successor. Since $\M$ is a $\GWCM$-model, there exists a world
$w^\star$ of $\M$ such that $\Ical(w) R w^\star$ and $e(\Ical(w),\Box
\a) = e(w^\star, \a)$.  Let $\Ical'=\reass{\Ical}{w_1}{w^\star}$.  We
...
\Ical(t)$ and $e(\Ical(w),\Box \a) = e(w^\star, \a)$, we have $e(w^\star, \a)\lt
\Ical(t)$, and this implies $\M\models_{\Ical'} w_1:\a \lt t$. Let
$\chi\in\Phibd{\G,w,w_1}$ and assume that $\chi=w_1 : \b \gt t'$.  We
...
\begin{proof}
We only consider two significant cases.
...
\[
  \begin{array}{l}
   \AXC{$\overbrace{w :\a > w : q,\,  w :\b \leq w :q,\,  w : q  < t,\,\D_w,\,\Theta}^{\G'}$}    
    ...
    \DP
  \\[2ex]
\mbox{$\Delta_w$ only contains constraints having label $w$}
 \\[1ex]
\mbox{$\Theta$ does not contain constraints having label $w$}
  \end{array}
\]
...
\[
  \begin{array}{lcl}
\wg{\G[w]}&\;=\; &\wg{w:\a\to \b < t} + \wg{\G_w} \;=\; \wg{\a} +  \wg{\b} + 1 +  \wg{\G_w} 
\\[1ex]
    \wg{\G[w']}&\;=\; & \wg{w :\a > w : q} +\wg{w :\b \leq w :q} + \wg{w : q  < t } + \wg{\G_w}
\\
               &\;=\;&  \wg{\a} +  \wg{\b} + 0 +  \wg{\G_w} 
  \end{array}
\]
Since $\wg{\G[w']} <  \wg{\G[w]}$, we get $\sizem{\G'} < \sizem{\G}$.


Let us consider an application of $\Box\lt_1$ having conclusion $\G$ and let
$\G'$ be the right premise. Then, we can represent the application as follows:
\[
  \begin{array}{l}
  % \AXC{$1 \lt t,\,\Phizo{\G,w}$}
  \AXC{\dots}
     \AXC{$\overbrace{R(w,w_1) > 0,\,  \D_{w_1}, \,\D_w,\,\Theta}^{\G'}$}
     % -----------------------------------------
     \RightLabel{$\Box\lt_1$}
     \BIC{$\underbrace{w:\Box\a \lt t,\,\Delta_w,\,\Theta}_\G$}    
    \DP
\\[8ex]
\mbox{$\Delta_w$ only contains constraints  having label $w$}
 \\[1ex]
...
& \G'[w] =\D_w,\; \G'[w_1]=\D_{w_1}
  \end{array}
\]  
To prove that $\G'\precc \G$, we show that:
...
$\wg{\G'[w_1]} < \wg{\G[w]}$.
\end{enumerate}
The proof of point~\ref{lemma:rulesDec:box1} is immediate since $\G'[w]$ is obtained
from  $\G[w]$ by deleting the constraint $w:\Box\a \lt t$.
...
\end{enumerate}
Let $\chi_1\in   \Phibd{\Delta_w,w,w_1}$; then there exists $\chi\in \Delta_w$ such that one of the two
