...
\item  Let $\Ical$ be an $\M$-interpretation, let $w$ be a label of $\LC$
  and $w^\star$ a world of $\M$.  By $\reass{\Ical}{w}{w^\star}$ we
  denote the $\M$-interpretation $\Ical'$ such that $\Ical'(w) =
...
\begin{proof}
  \todo{SISTEMARE}
  Let $\G$ be the conclusion of rule $\rho$ of $\Calcw$ and let us
  assume $\M\models_\Ical\G$, where $\M=\stru{W,R,e}$; we show that
  there exist a premise $\G'$ of $\rho$, a $\GWM$-model
  ...
  Let $\rho$ be application of $\to <$, where $\b\not\in \Vcal\cup{\bot}$:
  \[\small
  %% IMP LTS 
  ...
  < t$. Note that $q$ is a fresh propositional variable. Now let $r=e(\Ical(w),\b)$ and $\M'=\stru{W,R,e'}$ be the model where $e'=e[\Ical(w),q:=r]$.
  We remark taht, for every constraint $\chi$ not containing $q$,
  $\M\models_\Ical \chi$ iff $\M'\models_{\Ical} \chi$.  Since
  $\M\models_{\Ical} \G_0$ and $b$ does not occur in $\G_0$, we get
  $\M'\models_{\Ical} \G_0$.  It is easy to check that
  $\M'\models_{\Ical} w :\b \leq w:q$ and $\M'\models_{\Ical} w :\a >
  w:q$ and $\M'\models_{\Ical} w:q < t$, hence $\M'\models_{\Ical}
  \G_1$.
  
  Let $\rho$ be application of $\to \leq$, where $\b\not\in \Vcal\cup{\bot}$:
  \[
  %% IMP LTE 
...
hence $\M\models_\Ical w:\b \leq t$.  Note that $q$ is a fresh propositional variable.
Now let $r=e(\Ical(w),\b)$ and $\M'=\stru{W,R,e'}$ be the model where $e'=e[\Ical(w),q:=r]$.
...
Let $\rho$ be application of $\to \gt$, where $\b\not\in \Vcal\cup{\bot}$:
\[
%% IMP GT 
...
\b) = 1$, which implies $1 \gt \Ical(t)$. 
Now let $r=e(\Ical(w),\a)$ and $\M'=\stru{W,R,e'}$ be the model where $e'=e[\Ical(w),q:=r]$. 
We can prove that $\M'\models_{\Ical} \G_1$.

Let $\rho$ be application of $\Box\lt$:
\[
...
Let us assume that the world $\Ical(w)$ of $\M$ has no $R$-successors, then $e(\Ical(w),\Box\a)=1$ and $t\gt 1$. Furthermore $\Ical(R(w,w_1)=0$ and thus every formula of the form $\Ical(R(w,w_1)\to w_1:\d)=1$ and every formula of the form $\Ical(R(w,w_1)\wedge w_1:\d)=0$. Hence we get that $\M\models_{\Ical} \G_1$.
Otherwise if $\Ical(w)$ has at least one $R$-successor, since $\M$ is a $\GWM$-model, there exists a world
...
$\chi\in\Phibd{\G,w,w_1}$ and assume that $\chi=R(w,w_1)\to w_1 : \b \gt t'$. We have $w : \Box \b \gt t'\in \G_0$, hence $e(\Ical(w),\Box \b) \gt
\Ical(t')$.  Since $\Ical(w) R w^\star$, it follows that $R(\Ical(w),w^\star)\to e(w^\star,\b)\gt \Ical(t')$, and this implies $\M\models_{\Ical'} \chi$.
Similarly, if $\chi=R(w,w_1)\wedge w_1 : \b \lt t'$, namely $w : \Diam \b \lt t'\in
\G_0$, we get $\M\models_{\Ical'} \chi$.  We conclude
$\M\models_{\Ical'} \G_1$. The case where $\rho$ is  rule $\Diam\gt$ is similar.

Let us cionsider the application
\[
\AXC{$\overbrace{w':\a<R(w,w'),\,  w':\a  < t,\,  \G_0}^{\G_1}$}    
    ...
    \DP
\]
By the hypothesis, it holds that $\M\models_{\Ical} R(w,w')\to w': \a < t$
and $\M\models_{\Ical} \G_0$. 
This implies that $\Ical(R(w,w')\to
w':\a) < \Ical(t) \leq 1$, hence $R(\Ical(w),\Ical(w')) > e(\Ical(w'),\a)$ and
$\Ical(R(w,w')\to
w':\a)= e(\Ical(w'),\a)$, thus $\M\models_\Ical w':\a <
t$. Since $\M\models_{\Ical} \G_0$, we get $\M\models_{\Ical} \G_1$.  
\end{proof}

%R(\Ical(w),\Ical(w')) \to e(\Ical(w'),\a)
