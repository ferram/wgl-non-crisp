...
the construction succeeds, we say that the constraint $\psi=\Box\a$ is
checked, and we proceed by considering the next modal constraint in
$\Phi(w_0)$, which in turns generates a new world.  Note that, at each
iteration, one can discard the information related to the current new
world, so the space usage remains polynomial in the size of $\varphi$.
...
If all the non-determistic attempts to build a countermode fail,
