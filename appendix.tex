...
\begin{proof}
  ...
  Let $\rho$ be application of $\to <\;(\dag)$, where $\b\not\in \Vcal\cup{\bot}$:
  \[\small
  %% IMP LTS 
  ...
  < t$. Note that $q$ is a fresh propositional variable.
  
  Now let $\M'=\stru{W,R,e'}$ be the model where  
  

  Let $e(\Ical(w),\b)\leq e(\Ical'(w),q)$ and
  let, for every constraint $\chi$ not containing $q$,
...
Let us assume that the world $\Ical(w)$ of $\M$ has no $R$-successors, then $\Ical(R(w,w_1)=0$ and thus $\Ical(R(w,w_1)\to w_1:\a)=1$. Hence we get $1\lt t$ and $\M\models_{\Ical} \G_1$.
