...
non-deterministic procedure to check whether a model having root $w_1$
satisfying all the constraints in $\Phi(w_1)$ can be constructed.  If
the construction fails, we conclude that a countermodel for $\varphi$
cannot be built, hence $\varphi\in\GWL$. Otherwise, we proceed to the
next modal constraint in $\Phi(w_0)$ and generate a new successor of
$w_0$.  At this stage, we can discard all information related to
$w_1$, so the space usage remains polynomial in the size of $\varphi$.
If, for every modal constraint in $\Phi(w_0)$, the corresponding
construction succeeds, then a countermodel for $\varphi$ exists (where
$e(w_0,\varphi) = r_0$), hence $\varphi \notin \GWL$.
