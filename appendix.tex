...
We sketch a PSPACE procedure to check the membership of a formula to
...
$\Phi(w)$ is a set of equalities of the form $e(w,\a)=r$, we call
evaluations, where $\a$ is a subformula of $\varphi$ and
...
$\M=\stru{W,R,e}$ is coherent with $\Phi$ iff, for every $w$ in $W$
and every evaluation $e(w,\a)=r$ stored in $\Phi(w)$, the value of
...
\item Let $w$ be a world name and let us assume that $\Phi(w)\neq
  ...
  $\GWM$-model coherent with $\Phi$ can be built, $\Fail$ otherwise.
\end{itemize}
...
\item\label{step:1} We apply propositional saturation reductions to
  $\Phi(w)$ as long as possible. At each step, an evaluation
  $e(w,\a\star\b)=r$ (with $\star\in \{\,\land,\,\lor,\,\to\,\}$) in
  $\Phi(w)$ is removed and replaced by the evaluations $e(w,\a)=r_1$
...
\item\label{step:2} Let $e(w,\#_1\a_1)=r_1,\dots,e(w,\#_n\a_n)=r_n$
  ...
  the evaluations required to justify the evaluations
  ...
  stress that there at most $\size{\phi}$ nested recursive calls.  Let
  ...
  the the memory.  Let us assume that $\Search(w_k)$ returns $\Fail$.
  Then, a backtrack step is needed.  If no new $\Qcal$-choice is
  possible, the call $\Search(w)$ returns $\Fail$.
\end{enumerate}
The procedure  can be implemented so that
the $\Search(w)$ can be computed in space $O(\size{\varphi})$.

We can exploit $\Search$ to decide the validity of $\varphi$ in
$\GWL$.  We initialize $\Phi$ by setting $e(w_0,\varphi)= q_0$, where
