...
\begin{proof}
  ...
  \Ical(t)$. We have two cases.  If the world $\Ical(w)$ has no
  ...
  has at least one $R$-successor, since $\M$ is a $\GWM$-model, there
  exists a world $w^\star$ of $\M$ such that $R(\Ical(w),w^\star)\to
  e(w^\star,\a)=e(\Ical(w),\Box\a)$.  Let
  $\Ical'=\reass{\Ical}{w_1}{w^\star}$.  We remark that, for every
  ...
  \item for very $\chi=R(w,w_1)\to w_1 : \d \gt
    ...
    hence $e(\Ical(w),\Box \b) \gt \Ical(t')$.  Since $\Ical(w) R
    w^\star$, it follows that $R(\Ical(w),w^\star)\to e(w^\star,\b)\gt
  ...
  \item for every $\chi=R(w,w_1)\wedge w_1 : \b \lt
    ...
    hence $e(\Ical(w),\Diam \b) \lt \Ical(t')$. Since $\Ical(w) R
    w^\star$, it follows that $R(\Ical(w),w^\star)\land
...
\begin{proof}
  ...
  and~\ref{lemma:rulesDec:box2} it follows that $\sizem{\G'}\precm
  \sizem{\G}$, hence $\G'\precc \G$.
\end{proof}
