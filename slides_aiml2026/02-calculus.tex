...
$\Calc$ is inspired by the calculus $\TKGL$ for $\KGL$ presented in
 ...
 \item 
  ...
  % \begin{center}
  %   \medfont
  %   \tm{B{\'{\i}}lkov{\'{a}}, Frittella, Kozhemiachenko: Paraconsistent
  %   {G}{\"{o}}del modal logic. {IJCAR} (2022)}
  % \end{center}
  
  \SSK

  $\KGL$ is an extension of $\GWCL$ over a more expressive
  language, including an involutive negation and a
  co-implication. 

  % \SSK

  % $\KGL$ has the finite model property, hence $\KGL$-models
  % are witnessed.
...
\begin{frame}
  ...
  \begin{center}\medfont
    \begin{tabular}{p{28ex}|p{50ex}}
      \emb{Constraint} & \emm{Intuitive semantical reading}\\[4ex]
      $w_0 : \a \;\tm{<}\; w_1 : p $ & The value of formula $\a$ at world $w_0$  is \tm{$<$} than the value of  $p$ (prop.~var.) at world $w_2$ 
      \\[4ex]
      %%%%%%%%%%
      $w_0 :\a \;\tm{\geq}\; R(w_0,w_1)  $  &
       The value of formula $\a$ at world $w_0$  is \tm{$\geq$} than the value of  $R(w_0,w_1)$
      \\[4ex]  
      %%%%%%%%%%
      $ R(w_0,w_1) \to w_1: \a  \;\tm{>}\; 0$ 
      &
             The value of   $ R(w_0,w_1) \to r$, where $r$ is the value of $\a$ at $w_0$,     is \tm{$> 0$}
    \end{tabular}
  \end{center}
...
\begin{frame}
  \frametitle{Constraints semantics}

  \small
  
  Given a set of constraints $\G$ and a $\GWCL$-model
  $\M=\stru{W,R,e}$, \bfb{$\Mcal$ satisfies $\G$} ($\Mcal\models \G$) if there exists an  interpretation
  $\iota$  mapping each world label in $\G$ to a world in $W$
  such that  $\iota(u)\abstractorder\iota(t)$ for every constraint
  $u\abstractorder t\in\G$    (\emm{all constraints are
    simultaneously satisfied in model $\M$}). 


%  where $\iota(w:\alpha)=e(\iota(w),\alpha)$ (\emm{all constraints are
%    simultaneously satisfied in model $\M$}). 
  ...
  \bfb{Example}
  \[
  \G=\{ \,\gcbox{$w':p> 0.1$}\,,\,\ycbox{$w':\Box p$} \geq
  0.5\,,\,\ggbox{$w'':p\land q$} \leq 0.3\}
  \]
  \vspace{-5ex}
  \begin{center}
  ...
  %   \begin{array}{rcl}
  %   \iota(\G)&=&
  %   \{\iota(c_1)<0.5\,,\,\gcbox{$e(\iota(w'),p)$}>\iota(c_1)\,,\,\ycbox{$e(\iota(w'),\Box p)$}\geq 0.25
  %   \,,\,\ggbox{$e(\iota(w''):p\land q)$} \leq 0.3\}
  %   \\
  %   &=&\{0.4<0.5\,,\,\hspace{1.73ex}\gcbox{$e(w_0,p)$}>0.4\,,\,\hspace{4.4ex}\ycbox{$e(w_0,\Box p)$}\geq 0.25
  %   \,,\,\hspace{2.7ex}\ggbox{$e(w_2:p\land q)$} \leq 0.3\}
  %   \\
  %   &=&\{0.4<0.5\,,\,\hspace{1.7ex}\gcbox{$0.5$}>0.4\,,\,\hspace{9.4ex}\ycbox{$0.25\geq 0.25$}
  %   \,,\,\hspace{8.5ex}\ggbox{$0.3$} \leq 0.3\}
...
\begin{frame}
  ...
  {\small
    ...
    \begin{array}{c}
      ...
      $\provesgw $  \ycbox{$w:\varphi<1$}}
      \\[1ex] 
      \tr{\Downarrow}
      \\[1ex] 
        \mbox{there is no $\M$ s.t. $\M\models \ycbox{$w:\varphi <1$}$}\\
        \tr{\Downarrow}
        % \mbox{$\tr{\nexists\,(\M\,,\,w)}~\mbox{s.t.}~ e(w,\varphi)<1$}
        % \\
        %\tr{\Downarrow}
        \\[1ex] 
      \tb{\forall\,\M\,,\,\forall\,w}~e(w,\varphi)=1
      \\[.5ex]
...
\begin{frame}{The calculus}
  ...
  Rules of the calculus, read bottom-up, decompose a non-atomic constraint in a sound way
...
\begin{frame}{The calculus}
 ...
 \medfont $e(w, {\Box} \a) = {\inf}_{{x}\in W}\Big\{\,R(w,{x})\,{\to}\, e(x,\a)\,\Big\}$
