...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.8]
      ...
      \draw[line width=1.5pt,->] (-0.8,.3) to[out=50,in=-180,relative] (1.6,1);
      %% from ATOMIC_CONSTRAINT to ABSTRACT_ORDER
      \draw[line width=1.5pt,->] (6,.3) to[out=-50,in=-160,relative] (4.6,1);
      %% from CONSTRAINT left to C-TERM
      \draw[blue,line width=1.5pt,->] (-1.5,-0.1) to[out=-50,in=-145,relative] (-2,-1);
      %% from CONSTRAINT right to ATOMIC_C-TERM
      \draw[magenta,line width=1.5pt,->] (-.3,-0.1) to[out=30,in=200,relative] (4.2,-1.7);
      %% from ATOMIC_CONSTRAINT right to ATOMIC_C-TERM
      \draw[magenta,line width=1.5pt,->] (6.6,0) to[out=40,in=140,relative] (7.4,-1);
      %% from ATOMIC_CONSTRAINT left to ATOMIC_C-TERM
      \draw[magenta,line width=1.5pt,->] (5.5,0) to[out=10,in=-130,relative] (4.5,-1);
...
%  \small
 ...
 %\begin{center}
    \begin{tcolorbox}[colframe=blue!75!black,width=30em]
      \center
    \bfb{$\Mcal\models \G$  ($\Mcal$ satisfies $\G$)} iff:\\[2ex]
...
\end{tcolorbox}   
  ...
  \[\small
    \begin{array}{c}
      \small
      %% AND >
    \AXC{$\ycbox{$w: \a \;\tm{>}\; t$},\;  \ycbox{$w: \b \;\tm{>}\; t$},   \, \G$}   
    % ------------------------------------------      
    \RightLabel{$\tr{\land >}$}
