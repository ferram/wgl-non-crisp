...
\medskip
  \begin{tcgreenbox}[top=0ex]{}
    ...
    define a function \tm{$\s$ (substitution)} mapping:
    \[
        \tb{\fbox{$w:p$}}\mapsto r\in\Qrange\quad \tb{\fbox{$R(w,w')$}}\mapsto r\in\Qrange\quad \tb{w:\bot}\mapsto 0
    \]
...
\[\medfont
  ...
  \]
  is \tr{consistent}. Indeed, we can take the following substitution:
    \[\medfont
      \begin{array}{c}
...
\[
   ...
   \]
Note that $\Atmp{\G}$ only consists of atomic constraints
...
\qquad\text{\red not consistent}
    \end{array}
  \]
  % Indeed, there is no $\s$ such that:
  %  \[\medfont
  %    \tb{1} > \s(\ggbox{$w_1:p_1$})\,,\;\s(\ggbox{$w_1:  p_1$}) \leq 1\,,\; \s(\ggbox{$w_1:p_1$}) \geq 1
  %    \]
We conclude that there is no  $\GWL$-model $\M$ such that $\M\models \G$
