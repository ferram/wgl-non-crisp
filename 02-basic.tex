...
\noindent
The logic $\GCL$ is obtained by adding the axiom $(Cr)=\Box(\a\lor\b)\to (\Box \a \lor \Diam \b)$
...
Let $K_\infty$ be the formula $\Box \neg\neg \a\to \neg\neg \Box \a$.
We remark that a countermodel for  $K_\infty$ must contain infinitely many worlds.
One can check that   the instance  $\varphi=\Box \neg\neg p\to \neg\neg \Box p$ of   $K_\infty$
is not valid in the model $\M$ in Fig.~\ref{fig:nonWitn}, since  $e(w_0,\varphi) = 0$ (see, e.g.,~\cite{FerFioRod:2025});
being  $\M$   crisp, we conclude that the axiom schema $(K_\infty)$ is not valid in $\GCL$.
We prove that  $(K_\infty)$ is valid in $\GWL$.
Let $\M=\stru{W,R,e}$ be a witnessed $\GM$-model and $w\in W$.
If $e(w, \neg\neg \Box \a)=1$, we immediately get $e(w, K_\infty)=1$. Otherwise,
...
hence $e(w, K_\infty) = 1$. This proves  that
  $(K_\infty)$ is valid in $\GWL$.

Let $\varphi =\Box (p\lor q)\to (\Box p \lor \Diam q)$ be an instance of $Cr$;
we show that $\varphi\not\in\GWL$
...
Finally, we point out that, in all the mentioned modal logics, $\Box$ and $\Diam$ are not
interdefinable.
