...
to  $\GKL$ (see~\cite{RodriguezV:21}).
 ...
 We show that  $\GWCL$ is a proper extension of both $\GCL$ and   $\GWL$ which, in turn, are proper
 extensions of $\GKL$.
Let $\psi=\Box \neg\neg \a\to \neg\neg \Box \a$. 
In~\cite{FerFioRod:2025},
it is proved that   $\psi\in\GWCL$ and $\psi\not\in\GCL$.
We show that $\psi\in\GWL$.
Let $\M=\stru{W,R,e}$ be a witnessed $\GM$-model and $w\in W$; we prove that $e(w, \psi) = 1$.
If $e(w, \neg\neg \Box \a)=1$, we immediately get $e(w,\psi)=1$. Otherwise,
it holds that $e(w, \neg\neg \Box \a)=0$, hence $e(w, \Box \a)=0$.
Accordingly,  there exists  $w'\in W$ such that $R(w,w') \to e(w',\a) = 0$;
...
in the definition of $R$ and $e$ we only display the non-zero values.
