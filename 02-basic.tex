...
iff  $R(w, w')>0$.
Let us  assume that  $w$ has no $R$-successors; since  $R(w,w')=0$ for every $w'\in W$, 
  $e(w,\Box \a) = 1$ and $r(w,\Diam \a) = 0$.
A \GM-model $\stru{W,R,e}$ is
\emph{witnessed} iff every world $w$ having at least one $R$-successor
satisfies the following properties:
\begin{itemize}
\item if $e(w,\Box \a)=r$, then 
   there is $w'$   s.t.~ $R(w,w')>0$ and $R(w,w') \to e(w',\a) = r$;

\item if $e(w,\Diam \a)=r$, then
   there is $w'$  s.t.~$R(w,w')>0$ and $R(w,w')\land e(w'\a) = r$.
  
\end{itemize}
Thus, the values   $e(w,\Box\a)$  and $e(w,\Diam\a)$  are witnessed by the $R$-successor $w'$.
...
We stress that, if $w$ has no $R$-successors, then
$e(w,\Box\varphi)=1$ and $e(w,\Diam\varphi)=0$, even if such
values are not witnessed. 
% (these peculiar cases are not considered in~\cite{BilkovaFK:22}).
...
1$.
% In the sequel, by \GWM-model, we mean a witnessed \GM-model;
% by  \GWCM-model we mean a witnessed crisp model.
A model $\M=\stru{W,R,e}$ is \emph{discrete} if $W$ is
finite and the image of $e$ is a finite subset of $\Qrange$, the set
...
it holds that $e(w, \neg\neg \Box \a)=0$, and this implies $e(w, \Box \a)=0$.
Note that $w$ must have at least one $R$-successor, otherwise we would get  $e(w, \Box \a)=1$.
Hence, there exists a witnessing $w'\in W$ such that $R(w,w') >0$ and
$R(w,w') \to e(w',\a) = 0$. It follows that   $e(w',\a)=0$, hence  $e(w',\neg \neg \a)=0$.
We get $e(w,  \Box\neg \neg  \a)=0$,
...
in App.~\ref{app:noninterdBD} the proof is extended to all the other considered modal logics.
...
\\[10ex]
    \begin{array}{rcl}
     e(w_0,\Box (p\lor q))&=&\inf\{\, R(w_0,w_1)\to e(w_1,p\lor q)\,\}\,=\, 0.6 \to 0.5 \lor 0.6 \,=\,1
    \\[1ex]
    e(w_0,\Box p) &=&
    \inf\{\, R(w_0,w_1)\to e(w_1,p)\,\} \,=\,  0.6 \to 0.5   \,=\, 0.5
\\[1ex]
    e(w_0,\Diam q)&=&
    \sup\{\, R(w_0,w_1)\land e(w_1,q)\,\}\,=\, 0.6 \land 0.6 \,=\, 0.6
\\[1ex]
    e(w_0,\, \varphi) &=& 1\to (0.5\lor 0.6) \,=\, 0.6
\end{array}
    \end{array}
  \]
