...
indeed,   for every $w'\in W$, it holds that
$R(w_0,w')=0$,
...
The \GM-model $\M=\stru{W,R,e}$ in  Fig.~\ref{fig:nonWitn} is not witnessed
(in the definition of $R$ and $e$ only the  non-zero values are displayed);
indeed, $e(w_0,\Box p)=0$, but there is no $w_j\in W$ such that
...
On the other hand,   $\psi\not\in\GCL$.
Indeed, let us consider the instance  $\psi'= \Box \neg\neg p\to \neg\neg \Box p$ of $\psi$
and the  model $\M$ in Fig.~\ref{fig:nonWitn} 
One can check that $e(w_0,\psi') = 0$ (see~\cite{FerFioRod:2025});
since  $\M$ is crisp, we conclude that $\psi\not\in\GCL$.
