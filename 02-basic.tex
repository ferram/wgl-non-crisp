...
Let us consider the instance $\varphi =\Box (p\lor q)\to (\Box p \lor \Diam q)$  of $Cr$;
we show that $\varphi\not\in\GWL$
...
hence $e(w, Wt) = 1$. This proves  that
  $Wt$ is valid in $\GWL$.
Since  any finite model  is witnessed, 
  $Wt$ is valid in every finite $\GM$-model,
  thus a  a countermodel for  $Wt$ must be infinite.
