...
\begin{abstract}
  ...
  \begin{itemize}
\item
  AIML \url{https://events.illc.uva.nl/aiml2026/}, 29 June – 3 July 2026, Amsterdam

  \item 
    {\bf Abstract long papers submission: 20 February 2026}

    \item
  {\bf Full papers submission: 27 February 2026}

  

  \item
At most 12 pages (references excluded), with an optional technical appendix of up to 5 pages, together with a plain-text abstract of 100-200 words
  \end{itemize}
...
\smallskip
  ...
  This paper considers the bi-modal logic with both $\Box$ and
  $\Diamond$ arising from Kripke models with crisp accessibility whose
  propositions are valued over the standard G\"odel algebra
  $[0,1]$. Since this logic lacks the finite model property, we study
  the logic $\GWCL$ relying on \emph{witnessed} Kripke models where,
  for each modal formula, there is an assignment where the formula
  without the modality takes the same value as the modal one. We
  provide a cut-free sequent calculus and we exploit it to prove that
  $\GWCL$ is decidable and meets the finite model property.
  Finally, we explore a connection between the witnessed models and
  the well-known bi-relational Kripke semantics.
  % This paper considers the bi-modal logic with both $\Box$ and
  % $\Diamond$ arising from Kripke models with crisp accessibility whose
  % propositions are infinitely valued over the standard G\"odel algebra
  % $[0,1]$. Because this bi-modal logic arising from the mentioned
  % semantics does not satisfy the finite model property, we study the
  % bi-modal logic relying on witnessed Kripke modals (those in which,
  % for each modal formula, there is an assignment in the model where
  % the formula without the modality takes the same value as the modal
  % one). For the last bi-modal logic, we provide a cut-free sequent
  % calculus which will be sound and complete. On its basis, we
  % also prove their decidability for the considered bi-modal logic.
  % Finally, we explore a novel and simple connection between the
  % witnessed models and the well-known bi-relational Kripke semantics.
\end{abstract}
